...
NPM := npm
NPX := npx
ESBUILD_FLAGS := --bundle --jsx-import-source=preact --jsx=automatic
...
build-lib: ## Build only shared library
	cd ./packages/lib; $(NPX) esbuild index.ts --packages=external --outfile=./build/index.js --platform=neutral $(ESBUILD_FLAGS)
...
build-plugins: ## Build plugin library
	cd ./packages/plugins; $(NPX) esbuild index.ts --packages=external --outfile=./build/index.js --platform=neutral $(ESBUILD_FLAGS)
