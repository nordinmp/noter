...
build-lib: ## Build only shared library
	cd ./packages/lib; $(NPX) esbuild index.ts jsx.ts --packages=external --outdir=./build --platform=neutral $(ESBUILD_FLAGS)
...
build-plugins: ## Build plugin library
	cd ./packages/plugins; $(NPX) esbuild index.ts --packages=external --outfile=./build/index.js --platform=neutral --bundle $(ESBUILD_FLAGS)
...
build-cli: ## Builds CLI 
	cd ./packages/cli; $(NPX) esbuild index.ts --outfile=./build/cli.js --external:esbuild --platform=node --bundle $(ESBUILD_FLAGS)
